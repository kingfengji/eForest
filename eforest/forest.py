...
class BaseForest(six.with_metaclass(ABCMeta, BaseEnsemble)):
    ...
    def decode(self, X_encode, sample_method="minimal", null_value=0.):
        ...
        for single_encode in X_encode:
            ...
            single_decode = self.sample_X_by_spaces(spaces, sample_method=sample_method, null_value=null_value)
            X_decode.append(single_decode)
        X_decode = np.asarray(X_decode)
        return X_decode

    def get_paths_by_leafs(self, leafs):
        paths = []
        for i, bt in enumerate(self.estimators_):
            paths.append(bt.get_path_by_leaf(leafs[i]))
        return paths
    ...
    def get_spaces_by_leafs(self, leafs):
        ...
        """
        paths = self.get_paths_by_leafs(leafs)
        spaces = []
